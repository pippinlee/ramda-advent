...
const sortDim = (l,w,h) => R.sort(diff,[l,w,h])

// calculates extra paper need per present
const extraWrap = (sideOne,sideTwo) => sideOne*sideTwo

// calculate ribbon needed
const rib = (sideOne, sideTwo) => (parseInt(sideOne)+ parseInt(sideTwo))*2

// function that take a single present's dimensions
// output is total paper needed
const getDim = dim => {
  let dimSplit = R.split('x', dim)
  let l = dimSplit[0]
  let w = dimSplit[1]
  let h = dimSplit[2]

  // return array of sides in order
  let calcExtra = sortDim(l,w,h)

  console.log(calcExtra)

  // return ribbon needed
  let ribbon = rib(calcExtra[0], calcExtra[1])
  console.log('rib: ' + ribbon)

  // return bow ribbon needed
  let bow = R.product(R.map(parseInt, calcExtra))
  console.log('bow: ' + bow)
  // return total ribbon
  let finalRibbon = ribbon + bow

  // multiple two smallest sides
  let finalExtra = extraWrap(calcExtra[0], calcExtra[1])
...
const ribbonTotal = R.sum(R.map(getDim, content))
